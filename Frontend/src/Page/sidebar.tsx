...
const SidebarItem: React.FC<SidebarItemProps> = ({ icon: Icon, label, to, subMenu, color }) => {
  const [open, setOpen] = useState(false);
  return (
    <div className="flex flex-col">
      <div onClick={() => subMenu && setOpen(!open)} className={`flex items-center justify-between px-3 py-2 cursor-pointer hover:bg-gray-100 ${color || ""}`}>
  ...
  return (
    <aside className="w-64 border-r border-gray-200 flex flex-col bg-slate-50 flex-shrink-0 h-screen sticky top-0">
      <div className="p-4 flex items-center gap-2">
        <div className="w-8 h-8 bg-red-500 rounded-md flex items-center justify-center text-white font-bold">M</div>
        <span className="font-bold text-lg tracking-tight">Workspace Management</span>
      </div>
